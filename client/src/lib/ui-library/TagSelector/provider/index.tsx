...
import type { VisibilityConfig } from '../../types/shared';
...
export function useTagSelectorContext() {
  ...
  if (!ctx) throw new Error('TagSelectorProvider not mounted');
  return ctx;
}

export const TagSelectorProvider: React.FC<{
  children: ReactNode;
  ...
  const value = useMemo<TagSelectorContext>(() => ({
