...
export function useValidatedLanguage(initial?: AppLanguage) {
  const initialLang = initial || environment.DEFAULT_LANGUAGE;
  
  const validatedInitial = environment.AVAILABLE_LANGUAGES.includes(initialLang) 
    ? initialLang 
  ...
  const setValidatedLang = useCallback((nextLang: AppLanguage) => {
    if (environment.AVAILABLE_LANGUAGES.includes(nextLang)) {
      setLang(nextLang);
    } else {
...
export function useLanguageConfig(lang: AppLanguage) {
  ...
  return useMemo(() => {
    ...
    const languageConfigSource = optionalConfig?.LANGUAGE_CONFIG || environment.LANGUAGE_CONFIG;
    const availableLanguagesSource = optionalConfig?.AVAILABLE_LANGUAGES || environment.AVAILABLE_LANGUAGES;
    
    const config: LanguageConfig = languageConfigSource[lang] || languageConfigSource[environment.DEFAULT_LANGUAGE];
    return {
