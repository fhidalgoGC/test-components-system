# AcordionList - Mobile Responsive Version
...
├── css/
│   ├── index.ts
│   └── AcordionList.module.css    # Mobile-specific CSS
├── hooks/
│   ├── index.ts
│   └── useAcordionList.hook.ts    # Mobile touch logic
├── types/
│   ├── index.ts
│   └── AcordionList.type.ts       # Mobile-specific types
├── views/
│   ├── index.ts
│   └── AcordionList.view.tsx      # Mobile React component
...
```typescript
if (isMobile) {
  return <AcordionListMobile {...props} />;  // < 768px
}
return <AcordionListWeb {...props} />;        // >= 768px
```
