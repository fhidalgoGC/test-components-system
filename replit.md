...
- **LoginCard**: Authentication card component with dual configurations: `with-credentials` (providers + email/password) and `providers-only` (external auth only). Uses modular layout system with separate `WithCredentialsLayout` and `ProvidersOnlyLayout` components in dedicated `layouts/` folder. In `with-credentials` mode, credentials section appears first (email, password, remember me, continue button), followed by an "or" divider, then providers grid at the bottom. Supports up to 4 visible providers with expandable "more" button. All text labels (title, subtitle, provider labels) use `MultiLanguageLabel` interface for reactive i18n support. The `resolveLabel()` helper function resolves labels based on current language. In `providers-only` mode, providers display as large horizontal buttons with left-aligned icon + text. Fully responsive design with mobile/web variants.
