...
- **Data-Driven Components**: `BaseTable` (migrated to `shared/web/mobile` structure with dual platform support), `Paginator`, `List`, `Grid`, `AcordionList` offer declarative APIs and external state control. `BaseTable` supports infinite scroll via `behaviors.infiniteScroll` config (`enabled`, `threshold`, `loadingMoreMessage`, `loadingMoreComponent`), `callbacks.onReachEnd`, and `state: 'loadingMore'`. Includes re-entrancy guard and auto-load detection (fires `onReachEnd` when content doesn't overflow to auto-fill the container). Works on both web and mobile. Vertical scroll uses a single-div scroll container (`maxHeight` + `minHeight` + `overflowY: auto`) when `needsScrollContainer` is true (`heightMode: 'fixed'` + `verticalScroll: true` without `stickyHeader`). Demo page migrated to `ComponentLayoutView` tab pattern (Examples + Props tabs) at `pages/base-table-demo/web/`. Old demo files at `views/` and `components/` directories deleted.
